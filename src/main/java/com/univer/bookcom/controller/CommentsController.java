...
public class CommentsController {
    ...
    @Operation(summary = "Создать комментарий",
            responses = {
                    @ApiResponse(responseCode = "201", description = "Комментарий успешно создан",
                            content = @Content(schema = @Schema(implementation = CommentsResponseDto.class))),
    ...
    public ResponseEntity<CommentsResponseDto> createComment(
        ...
        CommentsResponseDto comment = commentsService.createComment(commentDto);
    ...
    public ResponseEntity<List<CommentsResponseDto>> getCommentsByBookId(
        ...
        List<CommentsResponseDto> comments = commentsService.getCommentsByBookId(bookId);
        if (comments.isEmpty()) {
    ...
    public ResponseEntity<List<CommentsResponseDto>> getCommentsByUserId(
        ...
        List<CommentsResponseDto> comments = commentsService.getCommentsByUserId(userId);
        if (comments.isEmpty()) {
    ...
    public ResponseEntity<CommentsResponseDto> updateComment(
            @PathVariable @Positive(message = "ID комментария должен быть положительным числом") Long commentId,
            @Valid @RequestBody CommentsRequestDto commentDto) {
        log.debug("Запрос на обновление комментария ID: {}", commentId);
        CommentsResponseDto updatedComment = commentsService.updateComment(commentId, commentDto);
